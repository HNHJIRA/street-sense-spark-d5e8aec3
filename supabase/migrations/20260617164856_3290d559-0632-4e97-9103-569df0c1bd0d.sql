...
-- Add Bellevue, WA as a supported city.
INSERT INTO public.cities (slug, name, timezone, default_zoom, center)
VALUES (
  'bellevue', 'Bellevue', 'America/Los_Angeles', 14,
...
-- Bellevue area-coverage counter (identical shape to arlington_area_counts).
CREATE OR REPLACE FUNCTION public.bellevue_area_counts(
  p_city_id uuid,
...
REVOKE EXECUTE ON FUNCTION public.bellevue_area_counts(uuid, double precision, double precision, double precision, double precision) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.bellevue_area_counts(uuid, double precision, double precision, double precision, double precision) TO service_role;
