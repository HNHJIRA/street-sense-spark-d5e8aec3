...
import {
  getBellevueCoverage,
  type BellevueAreaCoverage,
} from "@/lib/parking/bellevue-coverage.functions";
import { runAdminSync } from "@/lib/parking/admin.functions";
import { useState } from "react";

export const Route = createFileRoute("/admin/bellevue-coverage")({
  head: () => ({ meta: [{ title: "Bellevue Coverage · Admin" }] }),
  component: BellevueCoveragePage,
});
...
function classify(a: BellevueAreaCoverage): { label: string; color: string } {
...
function BellevueCoveragePage() {
  const fn = useServerFn(getBellevueCoverage);
  ...
  async function runSync() {
    setSyncing(true);
    try {
      await sync({ data: { citySlug: "bellevue" } });
      await q.refetch();
    } finally {
      setSyncing(false);
    }
  ...
  async function runDebugSync() {
    ...
    try {
      const res = await fetch("/api/public/admin/sync-bellevue?wait=1", { method: "GET" });
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      <header>
        <h2 style={{ fontSize: 22, fontWeight: 700 }}>Bellevue, WA Coverage</h2>
        <p style={{ color: "#475569", fontSize: 13, marginTop: 4, maxWidth: 760 }}>
          Verified open-data coverage across Bellevue neighborhoods. Bellevue
          publishes a Streets centerline layer (~10,629 polylines) but does
          NOT publish curb regulations, sign inventory, RPZ block-face data,
          or paid-parking rate areas as machine-readable open data. Paid
          street parking was approved on 2026-05-28 and is in pilot rollout.
          Every Bellevue segment therefore carries an explicit{" "}
          <strong>UNKNOWN</strong> rule — the parking engine never invents
          legality. Use the AI Sign Scanner to resolve unknown blocks at the
          curb. See{" "}
          <code>docs/bellevue-coverage-discovery.md</code> for the full
          dataset audit.
        </p>
      </header>
      ...
      <section style={{ border: "1px solid #e2e8f0", borderRadius: 8, background: "white", padding: 14 }}>
        ...
        {!debug && !debugError && (
          <div style={{ fontSize: 12, color: "#64748b" }}>
            ...
            <code>/api/public/admin/sync-bellevue?wait=1</code> and inspect the raw response.
          </div>
        )}
      ...
      <section>
        <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 8 }}>Provider Health (Bellevue)</h3>
      ...
      <section style={{ background: "#fffbeb", border: "1px solid #fcd34d", borderRadius: 8, padding: 14, fontSize: 12, color: "#78350f" }}>
        <div style={{ fontWeight: 700, marginBottom: 6 }}>Phase 3 — Future Data Sources (tracked, not yet built)</div>
        <ul style={{ margin: 0, paddingLeft: 18, display: "grid", gap: 4 }}>
          <li>
            <strong>Bellevue paid-parking GIS</strong> — Council approved
            paid street parking 2026-05-28 (rates up to $8/hr). No GIS feed
            published yet; rate-area boundaries currently exist only in PDF
            (curb-pricing implementation strategy, Nov 2025).
          </li>
          <li>
            <strong>Bellevue RPZ block-face GIS</strong> — Residential
            Permit Zones are managed through rpz.bellevuewa.gov as a permit
            purchase app. Zone polygons are not published. Will not scrape
            without an authoritative open-data feed.
          </li>
          <li>
            <strong>Bellevue curb management / CDS</strong> — 2023 Curb
            Management Plan commits to data-sharing pilots; no machine
            -readable curb-zone feed exists today.
          </li>
          <li>
            <strong>Arterial Sweeping Routes</strong> — schema carries a
            frequency code only (BikeHigh / ArterialsMedium / etc.). No
            day-of-week, no time-of-day. Cannot be turned into a
            street_cleaning window without inferring legality.
          </li>
        </ul>
      </section>

      <p style={{ fontSize: 11, color: "#64748b", maxWidth: 720 }}>
        Source datasets: City of Bellevue Open Data Hub — Streets (layer 10).
        Posted-sign curb regulations (no-parking, time-limited, loading,
        tow-away, permit, paid) are not comprehensively published by Bellevue,
        so Forecast and Can-I-Park-Here return UNKNOWN for blocks without
        verified rules. The AI Sign Scanner is the supported path to resolve
        them.
      </p>
    </div>
  );
...
function Stat({ label, value, danger }: { label: string; value: React.ReactNode; danger?: boolean }) {
  return (
    <div style={{ border: "1px solid #e2e8f0", borderRadius: 8, padding: "8px 10px", background: "#f8fafc" }}>
      ...
      <div style={{ fontSize: 16, fontWeight: 700, color: danger ? "#dc2626" : "#0f172a", marginTop: 2 }}>{value}</div>
    </div>
  );
}
