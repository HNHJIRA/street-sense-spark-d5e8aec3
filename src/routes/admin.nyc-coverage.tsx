...
import {
  getNycCoverage,
  type NycAreaCoverage,
} from "@/lib/parking/nyc-coverage.functions";
import { runAdminSync } from "@/lib/parking/admin.functions";
import { useState } from "react";

export const Route = createFileRoute("/admin/nyc-coverage")({
  head: () => ({ meta: [{ title: "NYC Coverage · Admin" }] }),
  component: NycCoveragePage,
});
...
function classify(a: NycAreaCoverage): { label: string; color: string } {
...
function NycCoveragePage() {
  const fn = useServerFn(getNycCoverage);
  ...
  async function runDebugSync() {
    ...
    try {
      const res = await fetch("/api/public/admin/sync-nyc?wait=1", { method: "GET" });
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      <header>
        <h2 style={{ fontSize: 22, fontWeight: 700 }}>New York City Coverage</h2>
        <p style={{ color: "#475569", fontSize: 13, marginTop: 4, maxWidth: 760 }}>
          Phase 1 (foundation) — NYC street_segments are populated from the
          authoritative NYC <strong>CSCL Street Centerline</strong>{" "}
          (~115k polylines, citywide). Every segment carries an explicit{" "}
          <strong>UNKNOWN</strong> rule (priority 900) until later phases
          attach authoritative regulation overlays (Parking Regulation
          Shapefile, Sign feed, ASP, meters, bus zones, curb typology,
          Open Streets). The engine never invents legality. See{" "}
          <code>docs/nyc-coverage-discovery.md</code> for the full dataset
          audit and provider plan.
        </p>
      </header>
      ...
      <section style={{ border: "1px solid #e2e8f0", borderRadius: 8, background: "white", padding: 14 }}>
        ...
        {!debug && !debugError && (
          <div style={{ fontSize: 12, color: "#64748b" }}>
            ...
            <code>/api/public/admin/sync-nyc?wait=1</code> and inspect the raw response.
          </div>
        )}
      ...
      <section>
        <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 8 }}>Provider Health (NYC)</h3>
      ...
      <section style={{ background: "#fffbeb", border: "1px solid #fcd34d", borderRadius: 8, padding: 14, fontSize: 12, color: "#78350f" }}>
        <div style={{ fontWeight: 700, marginBottom: 6 }}>Phase Roadmap (only Phase 1 is built)</div>
        <ul style={{ margin: 0, paddingLeft: 18, display: "grid", gap: 4 }}>
          <li><strong>Phase 1 (current):</strong> nyc-centerline + nyc-diagnostics → 100% gray baseline.</li>
          <li><strong>Phase 2:</strong> nyc-regulations (Parking Regulation Shapefile) — biggest single coverage win.</li>
          <li><strong>Phase 3:</strong> nyc-meters + nyc-asp + nyc-bus.</li>
          <li><strong>Phase 4:</strong> nyc-signs (point feed, snap to centerline).</li>
          <li><strong>Phase 5:</strong> nyc-curb (Manhattan CBD typology) + nyc-open-streets.</li>
          <li><strong>Phase 6:</strong> apply_nyc_derived_allowed RPC + engine specificity tiebreaker.</li>
        </ul>
      </section>

      <p style={{ fontSize: 11, color: "#64748b", maxWidth: 720 }}>
        Source dataset (Phase 1): NYC CSCL Street Centerline (FeatureServer/0).
        Until later phases attach authoritative parking regulations, every NYC
        segment intentionally returns UNKNOWN — the engine never fabricates
        parking legality.
      </p>
    </div>
  );
